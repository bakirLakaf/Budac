...
def start(update: Update, context: CallbackContext):
    ...
    update.message.reply_text("🎉 مرحبًا بك في متجرنا الإلكتروني!\nاختر خيارًا من القائمة:", reply_markup=reply_markup)
...
def contact_us(update: Update, context: CallbackContext):
    ...
    keyboard = [[InlineKeyboardButton(contact[3], url=contact[2] if contact[1] != "phone" else f"tel:{contact[2]}")] for contact in contacts]
    reply_markup = InlineKeyboardMarkup(keyboard)
