...
def check_main_admin(update: Update, context: CallbackContext) -> bool:
    user_id = update.effective_user.id
    if user_id != MAIN_ADMIN_ID:
        update.message.reply_text("هذه الوظيفة متاحة فقط للأدمن الرئيسي!")
        return False
    return True
...
def add_product(update: Update, context: CallbackContext):
    if not check_main_admin(update, context):
...
def set_order_status(update: Update, context: CallbackContext):
    ...
    # إشعار للزبون (يتطلب تكامل مع بوت الزبائن)
    # context.bot.send_message(chat_id=order_customer_id, text=f"تم تحديث حالة طلبيتك إلى: {status}")
...
def set_contact_info(update: Update, context: CallbackContext):
    if not check_main_admin(update, context):
        return ConversationHandler.END
    update.callback_query.message.reply_text("أدخل نوع الاتصال (مثل: phone, facebook):")
